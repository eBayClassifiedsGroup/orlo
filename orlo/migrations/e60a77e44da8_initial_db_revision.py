...
def upgrade():
    ...
    sa.Column('release_id', sqlalchemy_utils.types.uuid.UUIDType(length=16), nullable=True),
    sa.ForeignKeyConstraint(['release_id'], ['release.id'], ),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('id')
    )
    op.create_index(op.f('ix_package_stime'), 'package', ['stime'], unique=False)
    op.create_table('release_metadata',
    sa.Column('id', sqlalchemy_utils.types.uuid.UUIDType(length=16), nullable=False),
    sa.Column('release_id', sqlalchemy_utils.types.uuid.UUIDType(length=16), nullable=True),
    ...
    op.create_table('release_platform',
    sa.Column('release_id', sqlalchemy_utils.types.uuid.UUIDType(length=16), nullable=True),
    sa.Column('platform_id', sqlalchemy_utils.types.uuid.UUIDType(length=16), nullable=True),
    sa.ForeignKeyConstraint(['platform_id'], ['platform.id'], ),
    sa.ForeignKeyConstraint(['release_id'], ['release.id'], )
    )
    op.create_table('package_result',
    sa.Column('id', sqlalchemy_utils.types.uuid.UUIDType(length=16), nullable=False),
    sa.Column('content', sa.Text(), nullable=True),
    sa.Column('package_id', sqlalchemy_utils.types.uuid.UUIDType(length=16), nullable=True),
